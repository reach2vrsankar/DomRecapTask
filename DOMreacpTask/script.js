...
var data = [
    ...
    },
    {
        ...
        "Labeldata": ["Dal Rice","Curd Rice","Pongal","Pulihora","White Rice"]
    },
    {
        "id": "5",
        "type": "text",
    ...
    },
    {
        ...
        "type": "select",
        "LabelName":"State",
        "Labeldata": ["Andaman and Nicobar Islands", "Andhra Pradesh","Arunachal Pradesh", "Assam", "Bihar","Chandigarh","Chhattisgarh", "Dadra and Nagar Haveli","Daman and Diu","Delhi","Goa","Gujarat", "Haryana","Himachal Pradesh", "Jammu and Kashmir", "Jharkhand","Karnataka","Kerala","Lakshadweep","Madhya Pradesh","Maharashtra","Manipur","Meghalaya","Mizoram","Nagaland","Odisha","Puducherry","Punjab","Rajasthan","Sikkim","Tamil Nadu","Telangana","Tripura","Uttar Pradesh","Uttarakhand", "West Bengal"]
    },
    {
        "id": "8",
        "type": "text",
...
data.forEach(obj=>{
    ...
    col.append(label);
    if(obj.type==='text'){
        var inp_tag = createInput('input','text','form-control',obj.LabelName.split(' ').join(""),obj.LabelName.split(' ').join(""),'');
    ...
    if(obj.type==='radio'){
        ...
        for(var i=0;i<obj.Labeldata.length;i++){
            ...
            var inp_rad = createInput('input','radio','form-check-input',obj.LabelName.split(' ').join(""),obj.LabelName.split(' ').join(""),obj.Labeldata[i]);
    ...
    if(obj.type==='check'){
        ...
        for(var i=0;i<obj.Labeldata.length;i++){
            ...
            var inp_check = createInput('input','checkbox','form-check-input',obj.LabelName.split(' ').join(""),obj.LabelName.split(' ').join(""),obj.Labeldata[i]);
            var label = createEle('label','form-check-label');
            label.innerHTML = obj.Labeldata[i];
            label.setAttribute('for',obj.LabelName.split(' ').join(""));
